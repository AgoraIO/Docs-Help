...
To avoid volume changes when the user switches user roles, ensure that the volume type used by the SDK remains the same before and after the switch. The volume type used by the SDK varies from case to case. See [Volume type introduction](../integration-issues/system_volume/#volume-type-introduction) for details.
