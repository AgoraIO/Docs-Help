...
2. Check if the camera access permission is enabled. Both Android and iOS have a runtime access permission function under System Settings. See <a href="https://agoradocs.vercel.app/interactive-live-streaming/get-started/get-started-sdk#project-setup"> Add project permissions</a>.
...
3. Check whether the remote user uses the [channel encryption](voice-calling/develop/media-stream-encryption) function but the local user doesn't.
...
You can monitor the statistics of every call in [Agora Analytics](/agora-analytics/overview/product-overview) in Console. For more information, see [Agora Analytics Tutorial](/agora-analytics/reference/manage-agora-account).
