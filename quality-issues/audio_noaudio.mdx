...
platform: '["Voice","Video","Interactive Broadcast","Audio Broadcast","Interactive Gaming"]'
products: '无声,听不到,no audio'   
description: '' 
...
3. Allow Agora Video Call to access the microphone, and start a conversation to check whether they can hear each other.
   ...
   - If they cannot hear each other, proceed to step 2 to check the audio input device.
   	 <div class="alert note">On iOS, if a user equips a device with a non-Apple magnetic protection cover and cannot hear others, the issue may be caused by the protection cover. Remove the protection cover, leave the channel, and rejoin the channel to try whether the user can hear others.</div>
