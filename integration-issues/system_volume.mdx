...
| Echo cancellation algorithm | Uses the echo cancellation algorithm of the device system. | Uses the echo cancellation algorithm of the Agora <Vg k="VSDK" />. |

The system volume is the volume adjusted on the device, which is controlled by the volume type used by the Agora <Vg k="VSDK" />. For example, if the SDK uses the in-call volume, then when you adjust the system volume, you actually adjust the in-call volume.
