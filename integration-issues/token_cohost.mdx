...
<div class="alert warning"><ul>
	<li>Using co-host authentication requires app logic changes. Ensure that you read this article before enabling this function.</li>
	<li>Co-host authentication applies to scenarios where the channel profile is set to <code>LIVE_BROADCASTING</CODE>.</li>
	</ul></div>
...
- Uses only token-based authentication on all app clients to authenticate users. For details, see [Upgrade authentication mechanism](https://docs.agora.io/en/Interactive%20Broadcast/token?platform=Android#upgrade-authentication-mechanism).
