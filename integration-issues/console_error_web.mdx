...
| Reason | Solution    |
| :------------ | :----- |
| The web browser does not fully support WebRTC.  | Use a [Web browser](/video-calling/overview/supported-platforms?platform=web) supporting WebRTC. |
...
| The user's gateway firewall disables the UDP protocol or the UDP ports greater than 10000.  | Disable the firewall or use the [Agora Cloud Proxy](/voice-calling/core-functionality/cloud-proxy) service. |
...
- Use a supported [Web browser](/video-calling/reference/supported-platforms).
...
<div class="alert info">
  A candidate contains the IP address and port information for connecting to a
  remote device.
</div>

### Solution

The type of candidates used for connection depends on whether you have enabled [cloud proxy](/voice-calling/core-functionality/cloud-proxy) or not. Choose one of the following solutions accordingly.
...
- If you have not enabled cloud proxy, the SDK gets host candidates from the local device. In this case, the error is usually caused by the security settings of the local device.
  ...
  - Ensure that you have enabled UDP in the system firewall, and added the [specified domains and ports](/voice-calling/reference/firewall) to the whitelist.
