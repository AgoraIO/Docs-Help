...
<div class="alert note">This article applies to the Agora <Vg k="VSDK" /> Native SDK only.</div>

To apply optimization algorithms for different real-time engagement scenarios, Agora provides a `setChannelProfile` method for the <Vg k="VSDK" /> channel. You can use this method to set the channel profile as either `CHANNEL_PROFILE_COMMUNICATION` or `CHANNEL_PROFILE_LIVE_BROADCASTING`.
...
An <Vg k="VSDK" /> channel differentiates users by roles. Once a user joins a channel, the user can be either of the following:
...
<div class="alert note">
  ...
  <a href="../../help/integration-issues/token_cohost">How can I use co-host token authentication</a>.
</div>
