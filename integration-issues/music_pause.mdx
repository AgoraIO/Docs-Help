---
title: 'Why do music files not automatically resume playing after hanging up a system call on an Android device?'
...
After calling the `startAudioMixing` method, it is recommended to listen for phone events using the Android native method `PhoneStateListener` and perform the following steps:
