...
The section lists the browsers that the Agora <Vg k="VSDK" /> Web SDK supports on different platforms.
...
- On iOS 11 or later, the Web SDK supports Safari or later. However, Agora does not recommend using the Web SDK on iOS Safari due to the large amount of known issues and limitations of [iOS Safari](#safari). For better user experience on iOS, try the [Agora <Vg k="VSDK" /> iOS SDK](/en/sdks?platform=ios).
...
- Safari does not support enabling [dual-stream mode](../../signaling/reference/glossary#dual-stream-mode).
...
[Supported Platforms](../../video-calling/reference/supported-platforms)
